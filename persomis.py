...
with open('relationsMIS.txt', 'r') as fichier:
    relations = {}
...
def charger_personnages():
    personnages = {}
    with open('personnagesMIS.txt', 'r') as file:
        content = file.read()
        try:
...
for X, liste_relations in relations.items():
    for (Y, Z) in liste_relations:
        ...
        nom_y = personnages[Y]  # et du personnage pour Y
        G.add_node(nom_x)  # Ajouter le nœud X par son nom
        G.add_node(nom_y)  # et le nœud Y par son nom
